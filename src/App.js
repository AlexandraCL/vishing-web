...
// Función que genera un beep de alerta con Web Audio API
...
function SplashScreen({ onContinuar }) {
    return ( <
        ...
        <
        div style = {
            { width: 64, height: 64, borderRadius: 18, background: "rgba(255,255,255,0.12)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 32, fontSize: 32 }
        } > 🛡️ < /div> <
        h1 style = {
        ...
        <
        p style = {
            { color: "rgba(255,255,255,0.65)", fontSize: 15, lineHeight: 1.6, marginBottom: 32, textAlign: "center" }
        } >
...
function PanelPrincipalDesktop({ onNav, historialCount }) {
    ...
    return ( <
        ...
        h1 style = {
            { color: C.blanco, fontSize: 36, fontWeight: 800, marginBottom: 8 }
        } > Panel Principal < /h1> <
        p style = {
            { color: "rgba(255,255,255,0.55)", fontSize: 15, marginBottom: 40 }
        } > Monitorea tus llamadas y detecta posibles fraudes en tiempo real. < /p> <
        div style = {
            ...
            cards.map((c, i) => ( <
                ...
                p style = {
                    { color: C.blanco, fontWeight: 700, fontSize: 17, marginBottom: 8, lineHeight: 1.3 }
                } > { c.label } < /p> {
                c.count !== undefined && < p style = {
                    { color: C.blanco, fontWeight: 800, fontSize: 28, marginBottom: 4 }
                } > { c.count } < /p>} <
                p style = {
                    { color: "rgba(255,255,255,0.4)", fontSize: 13, marginBottom: 20 }
                } > { c.sub } < /p> <
                div style = {
                    { background: c.color, color: C.blanco, borderRadius: 12, padding: "10px 18px", textAlign: "center", fontSize: 14, fontWeight: 600 }
                } > { c.btn }→ <
                /div> < /
                div >
            ))
        ...
        div style = {
            ...
            [
                { label: "LLAMADAS ANALIZADAS", val: historialCount, color: C.blanco },
                { label: "RIESGO PROMEDIO", val: "0%", color: C.morado },
                { label: "ALERTAS EMITIDAS", val: "0", color: C.rojo },
            ].map((s, i) => ( <
                div key = { i }
                style = {
                    { background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: 20, textAlign: "center" }
                } >
                <
                p style = {
                    { color: "rgba(255,255,255,0.35)", fontSize: 12, marginBottom: 6, letterSpacing: "0.5px" }
                } > { s.label } < /p> <
                p style = {
                    { color: s.color, fontSize: 30, fontWeight: 800 }
                } > { s.val } < /p> < /
                div >
            ))
        } <
...
function PanelPrincipalMovil({ onNav, historialCount }) {
    ...
    return ( <
        ...
        <
        h2 style = {
            { fontSize: 22, fontWeight: 800, color: C.blanco, marginBottom: 4 }
        } > Panel Principal < /h2> <
        p style = {
        ...
        div style = {
            ...
            cards.map((c, i) => ( <
                ...
                /> <
                span style = {
                    { position: "absolute", right: 16, top: 16, fontSize: 28, opacity: 0.85 }
                } > { c.icon } < /span> <
                p style = {
                    { color: C.blanco, fontWeight: 700, fontSize: 16, marginBottom: 8, paddingRight: 48, lineHeight: 1.3 }
                } > { c.label } < /p> {
                c.count !== undefined && < p style = {
                    { color: C.blanco, fontWeight: 800, fontSize: 22, marginBottom: 2 }
                } > { c.count } < /p>} <
                p style = {
                    { color: "rgba(255,255,255,0.45)", fontSize: 11, marginBottom: 14 }
                } > { c.sub } < /p> <
                div style = {
                    { background: c.color, color: C.blanco, borderRadius: 10, padding: "8px 14px", display: "inline-block", fontSize: 13, fontWeight: 600 }
                } > { c.btn }→ <
                /div> < /
                div >
            ))
...
function Monitoreo({ onGuardar, mobile }) {
    ...
    const enviarSegmento = useCallback(async(blob) => {
        ...
        try {
            ...
            // Emitir alerta sonora si supera el umbral
            if ((data.riesgo || 0) >= 75) {
                emitirAlerta();
            }
            setTranscripcion(data.transcripcion || "");
            setPalabras((data.transcripcion || "").split(" ").filter(w => w.length > 5).slice(0, 8));
        } catch (e) {
    ...
    const iniciarLoop = useCallback(async() => {
        try {
            ...
            const grabarCiclo = () => {
                ...
                loopRef.current = setTimeout(() => { if (mr.state === "recording") mr.stop(); if (grabando) grabarCiclo(); }, 5000);
            };
            grabarCiclo();
        } catch {
    ...
    return ( <
        ...
        div > {
            ...
            <
            p style = {
                { fontWeight: 700, fontSize: 15, color: C.blanco, textAlign: "center", marginBottom: 16 }
            } > Nivel de riesgo < /p> <
            RiskGauge valor = { riesgo }
            /> {
            cargando && < p style = {
                { textAlign: "center", fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 10 }
            } > ⟳Analizando segmento... < /p>} {
            riesgo >= 75 && < div style = {
                { marginTop: 12, padding: "10px 14px", borderRadius: 12, background: "rgba(229,57,53,0.15)", border: "1px solid rgba(229,57,53,0.3)", textAlign: "center", color: "#ff8a80", fontWeight: 700, fontSize: 13 }
            } > ⚠️Alto riesgo de vishing detectado < /div>} < /
            div > {
                (transcripcion || palabras.length > 0) && ( <
                    ...
                    } > Palabras clave detectadas < /p> <
                    div style = {
                        { display: "flex", flexWrap: "wrap", gap: 8, marginBottom: transcripcion ? 14 : 0 }
                    } > {
                        palabras.map((w, i) => < span key = { i }
                            style = {
                                { background: "rgba(255,112,67,0.2)", color: "#ffab91", borderRadius: 8, padding: "4px 10px", fontSize: 13, fontWeight: 600 }
                            } > { w } < /span>)} < /
                            div > {
                                transcripcion && < p style = {
                        ...
                        /button> <
                        button onClick = {
                            () => onGuardar({ fecha: new Date().toLocaleString("es-MX"), estado: riesgo >= 75 ? "Sospechoso" : "Seguro", riesgo })
                        }
                        style = {
                            { width: "100%", height: 52, borderRadius: 26, background: C.morado, color: C.blanco, border: "none", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 16px ${C.morado}44` }
                        } >
                        Guardar resultado <
                        /button> < /
                        div > <
                ...
                function Historial({ lista, onLimpiar, mobile }) {
                    return ( <
                        ...
                        <
                        h2 style = {
                            { fontSize: mobile ? 22 : 28, fontWeight: 800, color: C.blanco, lineHeight: 1.2 }
                        } > Llamadas almacenadas < br / > hasta el momento < /h2> < /
                        div > <
                        div style = {
                            { padding: "0 16px" }
                        } > {
                            lista.length === 0 ? ( <
                                ...
                                } > No hay llamadas grabadas aún. < /p> < /
                                div >
                            ) : ( <
                                div style = {
                                    ...
                                    lista.map((item, i) => ( <
                                        ...
                                        <
                                        div style = {
                                            { width: 40, height: 40, borderRadius: 12, flexShrink: 0, background: item.estado === "Seguro" ? "rgba(0,150,136,0.2)" : "rgba(255,112,67,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }
                                        ...
                                        p style = {
                                            { fontSize: 13, fontWeight: 700, color: item.estado === "Seguro" ? C.verde : C.naranja }
                                        } > { item.estado }— { item.riesgo } % riesgo < /p> < /
                                        div > <
                                        /div>
                                    ))
                                } <
                        ...
                        style = {
                            { width: "100%", height: 56, borderRadius: 28, background: C.morado, color: C.blanco, border: "none", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: `0 4px 16px ${C.morado}44` }
                        } > 🗑Limpiar Historial <
                        /button> < /
                        div > <
                        /div>
                ...
                function Configuracion({ config, onChange, mobile }) {
                    ...
                    return ( <
                            ...
                            <
                            h2 style = {
                                { fontSize: mobile ? 22 : 28, fontWeight: 800, color: C.blanco }
                            } > Configuración < /h2> < /
                            div > <
                            div style = {
                                { padding: "0 16px" }
                            ...
                            <
                            div style = {
                                { width: 44, height: 44, borderRadius: 12, background: "rgba(123,97,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }
                            } > 📊 < /div> <
                            div >
                            <
                            p style = {
                                { fontWeight: 700, fontSize: 14, color: C.blanco }
                            } > Cambiar umbral de detección < /p> <
                            p style = {
                                { fontSize: 13, color: C.morado, fontWeight: 700 }
                            } > { config.umbral } % < /p> < /
                            div > <
                            /div> <
                            input type = "range"
                            ...
                            items.map(item => ( <
                                ...
                                <
                                div style = {
                                    { width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: `${item.color}22`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }
                                } > { item.icon } < /div> <
                                p style = {
                                    { flex: 1, fontWeight: 600, fontSize: 14, color: C.blanco }
                                } > { item.label } < /p> <
                                div onClick = {
                                    () => onChange(item.key, !config[item.key])
                                }
                                style = {
                        ...
                        } > Acerca de < /p> { ["Descripción del proyecto", "Créditos"].map((item, i) => ( <
                    div key = { i } >
                        <
                        p style = {
                            { padding: "10px 0", fontSize: 14, color: "rgba(255,255,255,0.5)", cursor: "pointer" }
                        }
                    onMouseEnter = { e => e.target.style.color = C.blanco }
                    onMouseLeave = { e => e.target.style.color = "rgba(255,255,255,0.5)" } > { item } < /p> {
                    i === 0 && < div style = {
...
export default function App() {
    ...
    const renderContenido = () => {
        if (pantalla === "configuracion") return <Configuracion config = { config }
        onChange = {
    ...
    return ( <
        ...
        {!mobile && ( <
                ...
                <
                div style = {
                    { width: 40, height: 40, borderRadius: 12, background: "rgba(255,255,255,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }
                } > 🛡️ < /div> <
                p style = {
                ...
                div style = {
                    ...
                    navItems.map(item => {
                        ...
                        return ( <
                            ...
                            span style = {
                                { fontSize: 11, fontWeight: activo ? 700 : 500, color: activo ? C.morado : "rgba(255,255,255,0.4)", transition: "color 0.2s" }
                            } > { item.label } < /span> {
                            activo && < div style = {
...

        {
            mobile && pantalla === "configuracion" && ( <
                ...
                style = {
                    { position: "fixed", top: 16, left: 16, background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 12, color: C.blanco, fontSize: 14, fontWeight: 600, cursor: "pointer", padding: "8px 14px" }
                } > ←Atrás < /button>
            )
        } <
        /div> < / >
